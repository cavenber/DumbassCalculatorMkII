# ***Dumbass Calculator Mark 2***
Calculator for Nerds
## About
An evolution to the original **Dumbass Calculator** I made a year ago, this *mark 2* goes beyond a python console app. It features a convenient and easy-to-understand user interface while not impacting the functionalites the original provided. Therefore, this new edition will prove vital to those with intelligence even lower. (or maybe you are just a huge nerd)
## Installation
To install ***Dumbass Calculator Mark 2*** onto your device. 
1. Ensure your device is an Android phone
2. Go to the latest release
3. Download the apk file
4. Open the apk file
5. Disregard the warnings Google Play gives you (If you suspect this program a virus. Please follow the instructions)
6. Wait for it to install
7. It should be installed
8. Congratulations
9. You did it
10. Why are you still here?
11. Go use ***Dumbass Calculator Mark 2***
12. Come on go try it out
13. Let's go
14. Go
15. Go
16. Go
17. Go
18. Go
19. Go
20. Go
21. Go
22. Go
23. Go
24. Go
25. Go
26. Go
27. Go
28. why are you like this
